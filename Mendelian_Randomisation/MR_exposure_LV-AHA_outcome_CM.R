...
aha_gwas_dir = paste0(root_dir, "gwas_raw/")

working_dir = paste0(
    root_dir, "/mr_2bp/Exposure_LV_Outcome_CM"
)
...
# keyname = 'WT'
# for (i in 1:16){
#     filename = paste0(aha_gwas_dir, "/40kahat64_sexagebmibsa2bp_", keyname, "_AHA_", i, "_chrmerged.regenie.gz")
...
for (gtrait in c("WT_Global", "Ecc_Global", "Err_Global")){
    ...
    {
        filename = paste0(
            aha_gwas_dir, "/AHA_40k_t14_2bp_full_", gtrait, "_chrmerged.regenie.gz"
        )
        run_MR_AHA_exposure(filename, gtrait, 0, log_file)
    },
    error = function(e) {
        print(e)
    }
